...
const { vantagens } = require('./src/vantagens')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Fuse\n' // Nome do bot
            + 'ORG:Uncover;\n' // Organização
            + 'TEL;type=CELL;type=VOICE;waid=5511988389618\n' //Seu número Whatsapp
            + 'END:VCARD'
prefix = '/'
blocked = []
limitawal = '999999999'
...
const getLevelingXp = (userId) => {
        ...
        const addLevelingLevel = (userId, amount) => {Lexa
            let position = false
            Object.keys(_level).forEach((i) => {
                if (_level[i].jid === userId) {
                    position = i
        ...
        const addLevelingId = (userId) => {
            ...
            fs.writeFileSync('./database/user/level.json', JSON.stringify(_level))
             ...
             if (position !== false) {
                ...
                let position = false
            Object.keys(_limit).forEach((i) => {Lexa
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD Fuse BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Fuse. Registre-se para fazer amizade com o bot Fuse por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Fuse|14\n\n──「 Fuse BOT 」──`,
			...
			const ownerNumber = ["5511988389618@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5511988389618@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5511988389618@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5511988389618@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5511988389618@s.whatsapp.net","559784155670@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net"]
...
if (text.includes("ip"))
  ...
  axios.get(`https://ipwhois.app/json/${aris}`).then((res) =>{ 
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("bosta")){
		...
		if (messagesC.includes("bot")){
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/Fusejokes.js');
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO Fuse BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Fuse domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'assinaturas': 
					client.sendMessage(from, assinaturas(prefix) , text, { quoted: mek })
					break
					case 'vantagens': 
					client.sendMessage(from, vantagens(prefix) , text, { quoted: mek })
					break
					case 'nekopoi':   
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Fuse ✨* 」\n*│+ wa.me/5511988389618*╰──────*「 *Fuse* 」*────*\n\n*_SE QUER SER ADMIN DO BOT Fuse_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
					client.sendMessage(from, iklan(prefix) , text, { quoted: mek })
					break
					case 'comandos':
					client.sendMessage(from, comandos(prefix) , text, { quoted: mek })
					break
					case 'premiumcek':
                    if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5511988389618*╰──────*「 * Fuse* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO Fuse BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				break
				case 'gojo':                 
				 data = fs.readFileSync('./src/gojo.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*gojo :V*')
				break
				case 'belle1':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do Fuse:*\n\n https://youtube.com/channel/UCZEtf9AlsC2zsJQwrfW-44w'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Fuse\n*YOUTUBE:* https://youtube.com/channel/UCZEtf9AlsC2zsJQwrfW-44w\n*WPP:* wa.me/+5511988389618\n*\n\nEspero que tenham gostado do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
